...
serve(async (req) => {
  console.log("Função facebook-sync foi chamada:", req.method, req.url);
  ...
  try {
    ...
    const { projectId, adAccountId, startDate, endDate } = requestData as SyncRequest;
...
async function fetchCampaigns(adAccountId: string, token: string, startDate: string, endDate: string, projectId: string) {
  ...
  for (const campaign of campaignsData.data) {
    ...
    if (!insightsResponse.ok) {
      console.error(`Erro ao buscar insights para campanha ${campaign.id}:`, await insightsResponse.text());
      continue;
    }
...
async function fetchAdSets(adAccountId: string, token: string, startDate: string, endDate: string, projectId: string) {
  ...
  if (!adsetsResponse.ok) {
    console.error("Erro ao buscar adsets:", await adsetsResponse.text());
    return { processed: [], total: 0 };
  }
  ...
  for (const adset of adsetsData.data) {
    ...
    if (!insightsResponse.ok) {
      console.error(`Erro ao buscar insights para adset ${adset.id}:`, await insightsResponse.text());
      continue;
    }
...
async function fetchAds(adAccountId: string, token: string, startDate: string, endDate: string, projectId: string) {
  ...
  if (!adsResponse.ok) {
    console.error("Erro ao buscar anúncios:", await adsResponse.text());
    return { processed: [], total: 0 };
  }
  ...
  for (const ad of adsData.data) {
    ...
    if (!insightsResponse.ok) {
      console.error(`Erro ao buscar insights para anúncio ${ad.id}:`, await insightsResponse.text());
      continue;
    }
